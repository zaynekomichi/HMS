...
function redirect_g_business(){
	...
	function history_render() {
		 var data = new FormData();
			 ...
			 var xhr = new XMLHttpRequest();
			        xhr.open('POST', "http://192.168.18.246/Ark/Lists/get_history.php", true);
			        xhr.onload = function () {
			          if (this.status==200) {
			    wrapper = document.getElementById("render_history");
			    wrapper.innerHTML = "";
					let results = JSON.parse(this.response);
					 if(results.length > 0){
			           for(var res of results){
		              	var line = document.createElement("table");
		                line.innerHTML = `<tr  class="his_special" data-id=${res['id']}><td>${res['start_date']}</td><td>${res['First_Name']}</td><td>${res['start_time']}</td><td>${res['Reason']}</td></tr>`;
			            wrapper.appendChild(line);
			              }
			            } else {
			              wrapper.innerHTML = "<span style='width: 700px; padding: 10px; margin-left: 300px;margin-right:300px;'><td>No Apointment History yet <span>";
			            }
			          }else {
